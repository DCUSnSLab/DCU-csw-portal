...
$sql_orderby = " order by wr_num asc, wr_reply asc";// 출력 순서 정의 desc or asc

$list_page_rows = $rows = 15;
...
$list = array();
$sql = " select a.*,b.wr_subject from (select * {$sql_common} {$sql_search} {$sql_orderby} limit {$from_record}, {$rows}) a left join {$g5['write_prefix']}{$board['bo_1']} b on a.sh_class = b.wr_id ";
...
$class_sql= sql_query(" select wr_subject, wr_id from {$g5['write_prefix']}{$board['bo_1']} where wr_is_comment = 0 order by sh_restartday asc ");
?>
...
<div id="sh_bo_list">
    ...
    <div class="list_top">
        ...
        <fieldset id="sh_bo_sch">
            ...
            <select name="class" id="class" class="first" onchange="fsearch.submit()">
                <option value="">교육과정 전체</option>
                <?php for ($i=0; $class_row=sql_fetch_array($class_sql); $i++) {?>
                <option value="<?= $class_row['wr_id'];?>" <?= get_selected($class,$class_row['wr_id']); ?>><?= $class_row['wr_subject'];?></option>
                <?php }?>
            </select>
            <div class="second">
    ...
    <form name="fboardlist" id="fboardlist" action="./board_list_update.php" onsubmit="return fboardlist_submit(this);" method="post">
            ...
            <table cellpadding="0" cellspacing="0">
                ...
                <thead>
                    <tr>
                        ...
                        <th scope="col">작성자</th>
                        <th scope="col" class="datetime">등록일</th>
                    </tr>
                </thead>
                <tbody>
                    <?php
                    ...
                    <tr>
                        ...
                        <td class="sv_use subject">
                            ...
                            <a href="<?php echo $list[$i]['ca_name_href'].$qstr ?>" class="cate_link"><span><?= $list[$i]['ca_name'] ?></span></a>
                            <?php } ?> 
        ...
        <div class="btn_area">
            <ul class="adm_btns">
                ...
                <?php if($is_admin){?>
                <li><button type="button" class="sh_adm_btn" onclick="window.open('<?php echo $board_skin_url ?>/sh_setup.php?bo_table=<?php echo $bo_table?>', 'cs_call', 'width=800px; height=<?php echo $member['mb_id'] == 'admin' ? '588' : '280'?>px; left=150; top=30px; scrollbars=yes');" >게시판관리</button></li>
                <?php if($board['bo_1']){?>
                <li><a href="/bbs/board.php?bo_table=<?= $board['bo_1'] ?>" class="sh_adm_btn">교육과정 내역</a></li>
                <?php }?>
                <?php } ?>
            </ul>
