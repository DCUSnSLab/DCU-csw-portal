...
if($board['bo_sort_field'] != 'sh_rank desc, wr_num, wr_reply' and $board['bo_6'] == '1'){ //출력순서 사용
	sql_query(" update g5_board set bo_sort_field = 'sh_rank desc, wr_num, wr_reply' where bo_table = '{$board['bo_table']}' ");
}else if($board['bo_sort_field'] == 'sh_rank desc, wr_num, wr_reply' and $board['bo_6'] != '1'){//출력순서 사용 안함
    ...
    <form name="fboardlist"  id="fboardlist" action="./board_list_update.php" onsubmit="return fboardlist_submit(this);" method="post">
        ...
        <ul id="sh_gall_ul" class="gall_row">
            ...
            <li class="gall_li col-gn-<?= $bo_gallery_cols ?>">
                ...
                <div class="gall_txt">
                    ...
                    <ul>
                        <?php if($list[$i]['sh_phone']){?>
                        <li><span>연락처</span><?php echo phoneNum($list[$i]['sh_phone']) ?></li>
                        <?php }?>
                        <?php if($list[$i]['wr_email']){?>
                        <li><span>이메일</span><?php echo $list[$i]['wr_email'] ?></li>
                        <?php } ?> 
                        <?php if($list[$i]['sh_class']){?>
                        <li><span>담당과목</span><?php echo $list[$i]['sh_class'] ?></li>
                        <?php } ?> 
                        <?php if($list[$i]['sh_schoolcareer']){?>
                        <li><span>학력</span><p><?php echo $list[$i]['sh_schoolcareer'] ?></p></li>
                        <?php } ?> 
                        <?php if($list[$i]['sh_career']){?>
                        <li><span>경력</span><p><?php echo $list[$i]['sh_career'] ?></p></li>
                        <?php } ?> 
                    </ul>
                </div>
            </li>
        ...
        <div class="btn_area">
            ...
            <ul class="adm_btns">
                ...
                <?php if($is_admin){?><li><button type="button" class="sh_adm_btn" onclick="window.open('<?php echo $board_skin_url ?>/sh_setup.php?bo_table=<?php echo $bo_table?>', 'cs_call', 'width=800px; height=<?php echo $member['mb_id'] == 'admin' ? '620' : '280'?>px; left=150; top=30px; scrollbars=yes');">게시판관리</button></li><?php } ?>
            </ul>
            <!-- 관리자 버튼 [e] -->
