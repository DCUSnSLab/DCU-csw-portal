...
// 주소 끝
if (!isset($write['sh_caption'])) {
    sql_query(" ALTER TABLE `{$write_table}` ADD `sh_caption` VARCHAR(255) NOT NULL DEFAULT '' AFTER `sh_addr3` ", false);
}
if (!isset($write['sh_video'])) {
    sql_query(" ALTER TABLE `{$write_table}` ADD `sh_video` TEXT NOT NULL DEFAULT '' AFTER `sh_addr3` ", false);
}
?>