...
<form name="fwrite" id="fwrite" action="<?php echo $board_skin_url ?>/sh_setup_update.php" method="post" enctype="multipart/form-data" autocomplete="off">
  ...
  <p class="basicset_title">admin 게시판 기본설정</p>
  <table class="basicset">
    <tr>
      <th colspan="4">게시판공통설정</th>
    </tr>
    <tr>
      <th><label for="bo_1">교육과정 게시판명</label></th>
      <td><input type="text" name="bo_1" class="frm_input" value="<?= $board['bo_1'] ?>"/></td>
    </tr>
    <tr>
      <th>분류 ( 구분자| )</th>
      <td>
  ...
  <?php }else{?>
  <input type="hidden" name="bo_1" value="<?php echo $board['bo_1']?>"/>
  <input type="hidden" name="bo_use_email" value="<?php echo $board['bo_use_email']?>"/>
  <input type="hidden" name="bo_category_list" value="<?php echo $board['bo_category_list']?>"/>
  <input type="hidden" name="bo_use_category" value="<?php echo $board['bo_use_category']?>"/>
  <?php }?>
