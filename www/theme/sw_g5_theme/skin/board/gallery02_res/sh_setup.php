...
<form name="fwrite" id="fwrite" action="<?= $board_skin_url ?>/sh_setup_update.php" method="post" enctype="multipart/form-data" autocomplete="off">
  ...
  <table class="basicset">
    ...
    <tr>
      <th>연락처</th>
      <td><input type="checkbox" name="bo_1" value="1" <?= get_checked($board['bo_1'],'1') ?> id="bo_1"/> <label for="bo_1">체크시 사용</label></td>
      <th>주소</th>
      <td><input type="checkbox" name="bo_2" value="1" <?= get_checked($board['bo_2'],'1') ?> id="bo_2"/> <label for="bo_2">체크시 사용</label></td>
    </tr>
    <tr>
      <th>이메일</th>
      <td><input type="checkbox" name="bo_3" value="1" <?= get_checked($board['bo_3'],'1') ?> id="bo_3"/> <label for="bo_3">체크시 사용</label>(메일발송여부 <input type="checkbox" name="bo_use_email" value="1" id="bo_use_email" <?php echo $board['bo_use_email']?'checked':''; ?>> 사용)</td>
      <th>출력순서</th>
      <td><input type="checkbox" name="bo_4" value="1" <?= get_checked($board['bo_4'],'1') ?> id="bo_4"/> <label for="bo_4">체크시 사용</label></td>
    </tr>
    <tr>
      <th>링크</th>
      <td><input type="checkbox" name="bo_5" value="1" <?= get_checked($board['bo_5'],'1') ?> id="bo_5"/> <label for="bo_5">체크시 사용</label></td>
      <th><label for="bo_upload_count">파일 업로드 개수</label></th>
      <td><input type="text" name="bo_upload_count" value="<?php echo $board['bo_upload_count'] ?>" id="bo_upload_count" required class="frm_input" size="4"></td>
    </tr>
    <tr>
      <th>분류 ( 구분자| )</th>
      <td colspan="3">
    ...
    <tr>
        <th>이미지 크기</th>
        <td colspan="3">
          · <label for="bo_gallery_width">이미지 폭</label> : <input type="text" name="bo_gallery_width" value="<?php echo $board['bo_gallery_width'] ?>"  class="frm_input" style="width:40px;" id="bo_gallery_width"> px &nbsp;&nbsp;&nbsp;
          · <label for="bo_gallery_height">이미지 높이</label> : <input type="text" name="bo_gallery_height" value="<?php echo $board['bo_gallery_height'] ?>"  class="frm_input" style="width:40px;" id="bo_gallery_height"> px &nbsp;&nbsp;&nbsp;
          · <label for="bo_gallery_cols">이미지 수</label> : 줄당 <input type="number" name="bo_gallery_cols" value="<?php echo $board['bo_gallery_cols'] ?>"  class="frm_input" style="width:40px;"  onclick="vlach(this)" onblur="vlach(this)" id="bo_gallery_cols"> 개
        </td>
    </tr>
    <tr>
      <th>에디터사용</th>
      <td colspan="3"><input type="checkbox" name="bo_use_dhtml_editor" value="1" <?php echo $board['bo_use_dhtml_editor']?'checked':''; ?> id="bo_use_dhtml_editor"> 체크시 사용</td>
    </tr>
