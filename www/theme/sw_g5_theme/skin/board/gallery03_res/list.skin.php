...
<div id="sh_bo_gall">
    ...
    <ul id="sh_gall_ul" class="gall_row">
		...
		<?php for ($i=0; $i<count($list); $i++) {
			$ex_sh_caption=explode("!@!",$list[$i]['sh_caption']); ?>
        <li class="gall_li">
            <div class="gall_chk">
				<?php if ($is_checkbox) { ?>
            ...
            <div class="gall_con">
                ...
                <div class="gall_txt">
                    ...
                    <div class="desc">            
                    <?php 
                    for($i2=0;$i2<6;$i2++){//for문 i값($i<6) 수정하면 칸 수 조절 가능합니다. write 수정시 view, list도 수정해주세요 (최대 10)
						if ($ex_sh_caption[$i2]) {?>
                        <dl>
                            <dt><?php echo cut_str($ex_sh_caption[$i2],"8",".."); ?></dt>
                            <dd><?php echo cut_str($ex_sh_caption[$i2+1],"22",".."); ?></dd>
                        </dl>
                    <?php 
						}
					$i2++;
					}
					$wr_content = str_replace('&nbsp;','',strip_tags($list[$i]['wr_content']));?>
					<?php if($wr_content){?><p class="desc_txt"><?=cut_str(strip_tags($list[$i]['wr_content']), 85, "...")?></p><?php }?>
                    </div>                               
                </div>                
            </div>
            <!-- gall_con [e] -->
    ...
    <div class="btn_area">
    	...
    	<ul class="adm_btns">
            ...
            <li><button type="button" class="sh_adm_btn" onclick="window.open('<?php echo $board_skin_url?>/sh_setup.php?bo_table=<?php echo $bo_table?>', 'cs_call', 'width=800px; height=620px; left=150; top=30px; scrollbars=yes');">게시판관리</button></li><?php } ?>
        </ul>
		<!-- 관리자 버튼 [e] -->
